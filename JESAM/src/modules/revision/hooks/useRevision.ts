...
export function manuscriptNeedsEditorToStartPostRevisionRound(m: Manuscript): boolean {
  ...
  const hasTerminalDecision = decision === "minor-revision" || decision === "major-revision" || decision === "reject";
...
export function useRevision() {
  ...
  const submitRevision = useCallback(
    async (
      ...
      const reviewerNotifs = isMajorRevision
        ? (lastRound?.invitations.filter((i) => i.status === "accepted") ?? []).map((inv) => ({
      ...
      const nextMeta = metadataPatch(manuscript, {
        ...
        notifications: [...baseNotifs, ...reviewerNotifs],
      ...
      if (ok) toast.success(`Revised manuscript submitted for ${manuscript.reference_code ?? manuscript.id}.`);
      return ok;
    },
    [save, user?.id]
  );
  ...
  const sendToAuthor = useCallback(
    async (
      ...
      const lastRound = [...rounds].sort((a, b) => b.round - a.round)[0];
      const extensionPolicyDays = lastRound?.editorDecision === "minor-revision" ? 7 : 14;
      ...
      if (ok) toast.success(`Editorial review submitted and sent to the author of ${ref}.`);
      return ok;
    },
    [save, user?.id]
  );
  ...
  const submitCheckingDecision = useCallback(
    async (
      ...
      const ref = manuscript.reference_code ?? manuscript.id;
      const nextStatus: ManuscriptStatus = decision === "approve" ? "In Layout" : "Revision Requested";
      const nextMeta = metadataPatch(manuscript, {
        checking_review: {
          summary: review.summary,
          majorConcerns: review.majorConcerns,
        ...
        ...(decision === "send-back"
          ? {
              revision_cycle: {
                rounds: manuscript.submission_metadata?.revision_cycle?.rounds ?? [],
  ...
  const grantExtension = useCallback(
    async (manuscript: Manuscript, reason: string) => {
      ...
      if (ok) toast.success(`Extension granted for ${manuscript.reference_code ?? manuscript.id}.`);
      return ok;
    },
    [save]
  );
