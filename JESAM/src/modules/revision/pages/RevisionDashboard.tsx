...
function computeRevisionDeadline(
  ...
  const triggers = auditLogs
    .filter((l) => l.action === "revision-sent-to-author" || l.action === "checking-sent-back")
    .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  let startDate: Date | null = null;
  if (triggers.length > 0) {
    startDate = new Date(triggers[0].createdAt);
  } else if (meta.checking_review?.decision === "send-back" && meta.checking_review.reviewedAt) {
...
export default function RevisionDashboard() {
  const { role } = useAuth();
  const { manuscripts, submitRevision, sendToAuthor, submitCheckingDecision, grantExtension } = useRevision();
  ...
  const selectedNeedsCheckingDecision = selected ? manuscriptNeedsCheckingDecision(selected) : false;
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="text-gray-600 mt-1">
            Manage author revisions and editorial decisions. After a <strong>minor revision</strong> decision, the Technical Editor reviews the manuscript and the author submits a revised file for final checking. After a <strong>major revision</strong> decision, the manuscript returns to a new peer review round.
          </p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-xs text-gray-500 mb-2">
            Manuscripts currently under revision or peer review. Completed manuscripts are removed automatically.
          </p>
                ...
                >
                  ...
                  <p className="text-xs mt-1">
                    {manuscriptNeedsEditorialReview(m) ? (
                      ...
                      <span className="text-teal-700 font-medium">Final checking required</span>
                    ) : needs ? (
            ...
            <>
              ...
              <div>
                ...
                {selectedNeedsAction && (() => {
                  ...
                  const colors = isOverdue
                    ? { wrap: "bg-red-50 border-red-200 text-red-800", badge: "bg-red-100 text-red-900" }
                    : isUrgent
                    ? { wrap: "bg-amber-50 border-amber-200 text-amber-800", badge: "bg-amber-100 text-amber-900" }
                    : { wrap: "bg-green-50 border-green-200 text-green-800", badge: "bg-green-100 text-green-900" };
                  ...
                  return (
                    <div className={`mt-2 flex items-center justify-between rounded-lg border px-4 py-2.5 text-sm ${colors.wrap}`}>
                      <span>
                        Revision due by{" "}
                        <strong>
                          {dl.deadline.toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" })}
                        </strong>
                      </span>
                      <span className={`ml-3 shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ${colors.badge}`}>
                        {label}
                      </span>
                    </div>
                  );
                ...
                {selectedPostRevisionPeerReview && (
                  <div className="text-sm text-gray-800 mt-3 rounded-lg bg-indigo-50 border border-indigo-200 px-4 py-3 space-y-2">
                    <p className="font-medium text-indigo-950">Back in peer review — major revision round</p>
                    {selected && manuscriptNeedsEditorToStartPostRevisionRound(selected) && (
                      <p>
                        The revised manuscript has been submitted. Editors must{" "}
                        <strong>open the next peer review round</strong> in the Peer Review workspace to begin
                        inviting reviewers for the new version.
                      </p>
                    )}
                    {selected &&
                      ...
                      selected.status === "Peer Review" && (
                        <p>
                          The new peer review round is open. Reviewers are being invited for the revised file.
                        </p>
                      )}
                    <p>
                      After reviews are submitted, the editor records a decision — minor revision, major revision, or reject.
                    </p>
                    {role === "author" && (
                      <p className="text-indigo-900">
                        You will be notified when the desk requires further action. Track your manuscript from{" "}
                    ...
                    {isEditorialUser && (
                      <p>
                        ...
                        to start the new round, invite reviewers, and record a decision once reviews are in.
                      </p>
                    )}
                  </div>
                )}
                {!selectedNeedsAction && selectedHasUploads && !selectedPostRevisionPeerReview && (
                  <p className="text-sm text-gray-700 mt-2 rounded-md bg-slate-100 border border-slate-200 px-3 py-2">
                    Revision history is on record. No further action is required at this stage — current status: <strong>{selected.status}</strong>.
                  </p>
                )}
              </div>

              {role === "author" && (() => {
                const rounds = selected.submission_metadata?.peer_review?.rounds ?? [];
                const decidedRounds = rounds
                  .filter((r) => r.submissions.length > 0 || r.editorDecision)
                  .sort((a, b) => a.round - b.round);
                const editorialReview = selected.submission_metadata?.editorial_review;
                const checkingReview = selected.submission_metadata?.checking_review;
                if (decidedRounds.length === 0 && !editorialReview && !checkingReview) return null;
                ...
                return (
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-3">Peer review feedback</h3>
                    <div className="space-y-4">
                      {decidedRounds.map((r) => (
                        <div key={r.round} className="border border-indigo-100 rounded-lg overflow-hidden">
                          <div className="bg-indigo-50 px-4 py-2 flex items-center justify-between">
                            <p className="text-sm font-medium text-indigo-900">Round {r.round}</p>
                            {r.editorDecision && (
                              <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-800">
                                Decision: {decisionLabel[r.editorDecision] ?? r.editorDecision}
                              </span>
                            )}
                          </div>
                          {r.submissions.length > 0 ? (
                              ...
                              {r.submissions.map((s, i) => (
                                <div key={s.id} className="px-4 py-3 space-y-2">
                                  <div className="flex items-center justify-between">
                                    <p className="text-xs font-medium text-gray-700">Reviewer {i + 1}</p>
                                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">
                                      {recommendationLabel[s.recommendation] ?? s.recommendation}
                                    </span>
                                  </div>
                                  {s.summary && (
                                    <div>
                                      <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Summary</p>
                                      <p className="text-xs text-gray-700 whitespace-pre-wrap">{s.summary}</p>
                                    </div>
                                  )}
                                  {s.majorConcerns && (
                                    <div>
                                      <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Major concerns</p>
                                      <p className="text-xs text-gray-700 whitespace-pre-wrap">{s.majorConcerns}</p>
                                    </div>
                                  )}
                                  {s.minorConcerns && (
                                    <div>
                                      <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Minor concerns</p>
                                      <p className="text-xs text-gray-700 whitespace-pre-wrap">{s.minorConcerns}</p>
                                    </div>
                                  )}
                                </div>
                              ))}
                            ...
                            <p className="px-4 py-3 text-xs text-gray-500">No reviewer submissions yet for this round.</p>
                          )}
                        </div>
                      ))}
                      ...
                      {editorialReview && (
                        <div className="border border-purple-100 rounded-lg overflow-hidden">
                          <div className="bg-purple-50 px-4 py-2">
                            <p className="text-sm font-medium text-purple-900">Editorial Review</p>
                            {editorialReview.reviewedAt && (
                              <p className="text-[10px] text-purple-500 mt-0.5">
                                {new Date(editorialReview.reviewedAt).toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" })}
                              </p>
                            )}
                          </div>
                          <div className="px-4 py-3 space-y-2">
                            {editorialReview.summary && (
                              <div>
                                <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Summary</p>
                                <p className="text-xs text-gray-700 whitespace-pre-wrap">{editorialReview.summary}</p>
                              </div>
                            )}
                            {editorialReview.majorConcerns && (
                              <div>
                                <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Major concerns</p>
                                <p className="text-xs text-gray-700 whitespace-pre-wrap">{editorialReview.majorConcerns}</p>
                              </div>
                            )}
                            {editorialReview.minorConcerns && (
                              <div>
                                <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Minor concerns</p>
                                <p className="text-xs text-gray-700 whitespace-pre-wrap">{editorialReview.minorConcerns}</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {checkingReview && (
                        <div className="border border-teal-100 rounded-lg overflow-hidden">
                          <div className="bg-teal-50 px-4 py-2 flex items-center justify-between">
                            <p className="text-sm font-medium text-teal-900">Final Check</p>
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${checkingReview.decision === "approve" ? "bg-green-100 text-green-800" : "bg-orange-100 text-orange-800"}`}>
                              {checkingReview.decision === "approve" ? "Approved" : "Sent back"}
                            </span>
                          </div>
                          <div className="px-4 py-3 space-y-2">
                            {checkingReview.summary && (
                              <div>
                                <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Summary</p>
                                <p className="text-xs text-gray-700 whitespace-pre-wrap">{checkingReview.summary}</p>
                              </div>
                            )}
                            {checkingReview.majorConcerns && (
                              <div>
                                <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Major concerns</p>
                                <p className="text-xs text-gray-700 whitespace-pre-wrap">{checkingReview.majorConcerns}</p>
                              </div>
                            )}
                            {checkingReview.minorConcerns && (
                              <div>
                                <p className="text-[10px] uppercase tracking-wide text-gray-400 mb-0.5">Minor concerns</p>
                                <p className="text-xs text-gray-700 whitespace-pre-wrap">{checkingReview.minorConcerns}</p>
                              </div>
                            )}
                          </div>
                        </div>
                                ...
                                <div key={s.id} className="px-4 py-3 space-y-2">
                                  <div className="flex items-center justify-between">
                                    <div>
                                      <p className="text-xs font-medium text-gray-700">
                                        {r.invitations.find((inv) => inv.id === s.invitationId)?.reviewerName || `Reviewer ${i + 1}`}
                                      </p>
                                      <p className="text-[10px] text-gray-400">{s.reviewerEmail}</p>
                                    </div>
                                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">
                                      {recommendationLabel[s.recommendation] ?? s.recommendation}
                                    </span>
                                  ...
                                  {s.confidentialToEditor?.trim() ? (
                                    <div className="rounded-md bg-amber-50 border border-amber-200 px-3 py-2 mt-1">
                                      <p className="text-[10px] uppercase tracking-wide text-amber-700 mb-0.5 font-semibold">Confidential to editor</p>
                                      <p className="text-xs text-amber-900 whitespace-pre-wrap">{s.confidentialToEditor}</p>
                                    </div>
                                  ) : null}
                                </div>
                              ))}
              ...
              {role === "author" && selectedNeedsAction && (
                <div className="space-y-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-700 mb-1">Revised manuscript file (PDF)</p>
              ...
              {selectedNeedsEditorialReview && !isTechnicalEditor && (
                <div className="rounded-lg border border-purple-100 bg-purple-50/60 px-4 py-3 text-sm text-purple-800">
                  ...
                  <p className="mt-1">This manuscript is awaiting editorial review by the Technical Editor before it can be returned to the author.</p>
                </div>
              )}
                ...
                <div className="rounded-lg border border-purple-200 bg-purple-50 px-4 py-5 space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-purple-800 mt-1">
                      Complete the editorial review below. Submitting will notify the author and open the revision window.
                    </p>
                  </div>
                  ...
                  <button
                    type="button"
                    disabled={!editorialSummary.trim() || !editorialMajor.trim()}
                    onClick={() =>
              ...
              {selectedNeedsCheckingDecision && !isTechnicalEditor && isEditorialUser && (
                <div className="rounded-lg border border-teal-100 bg-teal-50/60 px-4 py-3 text-sm text-teal-800">
                  ...
                  <p className="mt-1">This manuscript is awaiting the Technical Editor's final 1-week check before it can be accepted or returned.</p>
                </div>
              )}
              ...
              {isTechnicalEditor && selectedNeedsCheckingDecision && (
                <div className="rounded-lg border border-teal-200 bg-teal-50 px-4 py-5 space-y-4">
                  <div>
                    <h3 className="font-semibold text-teal-900">Final Checking Review</h3>
                    <p className="text-sm text-teal-800 mt-1">
                      The author has submitted their revised manuscript. Complete the final review and decide to approve or return for further revisions.
                    </p>
                  </div>
                  <label className="block text-sm text-gray-700">
                    Summary
                  ...
                  <div className="flex gap-3 flex-wrap">
                    <button
                      ...
                      onClick={() =>
                        void submitCheckingDecision(
                          selected,
                          "approve",
                          { summary: checkingSummary, majorConcerns: checkingMajor, minorConcerns: checkingMinor }
                        ).then((ok) => {
                          if (ok) { setCheckingSummary(""); setCheckingMajor(""); setCheckingMinor(""); }
                        })
                      }
              ...
              {isEditorialUser && selectedNeedsAction && (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void grantExtension(selected, extensionReason).then((ok) => {
