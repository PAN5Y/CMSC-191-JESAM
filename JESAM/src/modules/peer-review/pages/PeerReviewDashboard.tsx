...
export default function PeerReviewDashboard() {
  ...
  const [editorialReviewSubmitting, setEditorialReviewSubmitting] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const runAutomations = async () => {
      ...
      for (const m of manuscripts) {
        ...
        const existingInvitations = roundData.invitations ?? [];
          ...
          // 2. Automated Reminders for Confirmed Reviewers within 3 days of deadline
            ...
            if (now > reminderThreshold) {
              ...
              if (!reminderAlreadySent && !cancelled) {
                await sendReviewReminder(m, inv.id);
                remindersSent++;
              }
            }
          }
  ...
  }, [manuscripts, sendReviewReminder, poolReady, reviewerPool, addInvitation]);
  ...
  const handleCompleteEditorialReview = async () => {
    ...
    if (!editorialReviewNote.trim()) {
      toast.error(
        "Add a note summarizing the editorial review before completing.",
      );
      return;
    }
    ...
    try {
      ...
      if (ok) {
        toast.success(
          "Editorial review complete. Manuscript accepted — author notified.",
        );
              ...
              {selected.status === "Editorial Review" ? (
                <div className="space-y-3 rounded-xl border border-gray-200 bg-white shadow-sm p-4">
                  ...
                  <p className="text-sm text-gray-600">
                    Peer review approved. Conduct your{" "}
                    <strong>1-week editorial review</strong> of all submitted
                    feedback before formally notifying authors. Once complete,
                    save your notes below and click{" "}
                    <strong>Accept Manuscript</strong> — the manuscript will
                    move to <em>Accepted</em> and an author notification will be
                    logged.
                  </p>
                  <div className="space-y-3">
                    ...
                    <button
                      type="button"
                      disabled={
                        !editorialReviewNote.trim() || editorialReviewSubmitting
                      }
