-- Fix 42P17 "infinite recursion detected in policy for relation peer_review_rounds".
-- Cause: peer_review_rounds policies queried reviewer_invitations, and invitation policies
-- queried peer_review_rounds. Use SECURITY DEFINER helpers so existence checks bypass RLS.
...
DROP FUNCTION IF EXISTS public.peer_review_is_staff();
DROP FUNCTION IF EXISTS public.peer_review_round_row_visible(uuid);
DROP FUNCTION IF EXISTS public.reviewer_invitation_row_visible_to_author(uuid);
DROP FUNCTION IF EXISTS public.review_submission_row_visible_to_author(uuid);
DROP FUNCTION IF EXISTS public.review_submission_invitation_matches_reviewer(uuid);

-- Staff check (reads profiles without triggering peer-review recursion)
...
-- SELECT allowed on a peer_review_rounds row: manuscript author OR invited reviewer email
